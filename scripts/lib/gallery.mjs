...
// Themes that support both schemes show light and dark previews side by side;
...
function renderPreview(extension) {
  ...
  if (extension.colorScheme === 'both' && patterns.length >= 2) {
    return `<div class="preview">${renderSwatch(patterns[0], 'light', alt)}${renderSwatch(patterns[1], 'dark', alt)}</div>`;
  }

  const variant = extension.colorScheme === 'dark' ? 'dark' : 'light';
  return `<div class="preview">${renderSwatch(patterns[0], variant, alt)}</div>`;
}
